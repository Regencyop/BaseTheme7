...
function addtocartpreview() {
    ...
    function template() {
        return [
            ...
            '.section-top div:nth-of-type(1) {padding:9px 0 0 4px;}',
            '.section-top div:nth-of-type(2) {float:left;max-width:100%; text-align:left; line-height:1.1em;}',
            ...
            '.section-bottom div.qty {margin:0 -5px; padding: 9px 0; background: #efefef; border: 1px solid #ccc}',
            '.section-bottom .pull-left .btn {float:left; margin: 5px; padding: 3px 12px; min-height: 20px; font-size:90%;}',
            '.section-bottom input.qty {margin: 0 auto; padding: 6px 1px; text-align: center}',
            '.sub-content {padding-right: 0.5em; text-align: right}',
            ...
            '</button>',
            '</div>',
            '<p class="col-xs-12 instruction"><span>Add all the versions of this item you need to your list, then click the Add List to Cart button to submit them all at once!</span></p>',
            '</div>',
            '<div>',
            ...
            '<img class="img-responsive" ng-src="{{item.Variant.LargeImageUrl}}"/>',
            '</div>',
            '<div class="col-xs-7 col-sm-8">',
            '<h4>{{item.Variant.Description || item.Product.Name}}</h4>',
            '<h6 ng-repeat="spec in item.Specs | filter:{\'CanSetForLineItem\':true} " class="spec-group-item spec-list">',
            '<span class="text-info spec {{spec.Label}}">{{spec.Label || spec.Name}}: </span>',
            ...
            '<div class="col-xs-5">',
            '<div class="qty text-center col-xs-4">Qty</div>',
            '<div class="col-xs-6"><input id="451qa_input_qty" class="qty form-control" ng-class="{\'has-error\': item.hasError}"  type="text" ng-model="item.Quantity" ng-change="qtyChanged(item)" /></div>',
            '</div>',
            '<div class="col-xs-6 pull-right sub-content">',
            '<h6>Unit Price: {{item.UnitPrice | culturecurrency}}<br />',
            'Total: {{item.LineTotal | culturecurrency}}</h5>',
            '</div>',
            '</div>',
            '<div class="list-item">',
